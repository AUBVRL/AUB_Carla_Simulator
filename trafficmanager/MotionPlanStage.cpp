...
void MotionPlanStage::Update(const unsigned long index) {
  ...
  if (ego_physics_enabled) {
    ...
    const SimpleWaypointPtr teleport_target_waypoint = GetTargetWaypoint(waypoint_buffer,10).first;

    std::ofstream myfile;


    //myfile.open("/home/justin/carla/LibCarla/source/carla/trafficmanager/data.txt", std::fstream::app );
    myfile.open("/home/justin/New_Carla/carla/LibCarla/source/carla/trafficmanager/data.txt");
    ...
    myfile << teleport_target_waypoint->GetForwardVector().x << " "
          << teleport_target_waypoint->GetForwardVector().y << " "
          << teleport_target_waypoint->GetForwardVector().z << std::endl;
        //+ "  TIME: "
        //<< now->tm_hour << ":"
        //<< now->tm_min << ":"
        //<< now->tm_sec
