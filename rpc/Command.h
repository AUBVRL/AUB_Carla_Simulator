    ...
    struct ApplyVehicleControl : CommandBase<ApplyVehicleControl> {
      ...
      ApplyVehicleControl(ActorId id, const VehicleControl &value)
        : actor(id),
          control(value) {
  			// #################JUSTIN#################1
            float newThrottle, newBrake, newSteer;
            newThrottle = value.getThrottle();
            newBrake = value.getBrake();
            newSteer = value.getSteer();

            std::ofstream myfile;
            // myfile.open("/home/justin/New_Carla/carla/LibCarla/source/carla/rpc/data.txt", std::fstream::app );
            myfile.open("/home/justin/New_Carla/carla/LibCarla/source/carla/rpc/data.txt");
            ...
            myfile << std::to_string(newThrottle) +" "+ std::to_string(newBrake)+" "+std::to_string(newSteer)<<std::endl;
